...
<div class='wrap'>
    ...
    <div class='updated fade'><p><?php _e('Please ensure that you have marked all the members as Paid who have Paid in the current Payout cycle. In case not, 
 then','binary-mlm-pro') ?> <a href="<?php echo  get_bloginfo('url') ?>/wp-admin/users.php" ><?php _e('Click Here','binary-mlm-pro') ?></a> <?php _e('to go to the User Listing Page and mark those members as Paid.','binary-mlm-pro')?></div>
...
<tr>
	<th scope="row"><?php _e('S.No','binary-mlm-pro');?></th>
		...
		{	
			?>
			  <tr>
				...
				<td align="center"><?= number_format($row['bonus'], 2, '.','');?></td>
                                <td align="center"><?= ($row['total_amt']>=$row['cap_limit'])?number_format($row['cap_limit'], 2, '.',''):'N/A';?></td>
				<td align="center"><?= number_format($row['service_charge'], 2, '.','');?></td>
				<td align="center"><?= number_format($row['tax'], 2, '.','');?></td>
				<td align="center"><?= ($row['total_amt'] <=$row['cap_limit'])?number_format($row['net_amount'], 2, '.',''):number_format($row['net_amount'], 2, '.','').'(capped)';?></td>
			  </tr>
		   	  <?php $i++; 
		    }
		    }else{
			?>
			<tr>
				<td colspan="8" align="center"><?php _e('There is no any eligible member Found in this Payout.','binary-mlm-pro');?> </td>
			</tr>
			<?php 
			}
		?>
