...
<a class="button" style="text-decoration: none;" href="<?php echo admin_url()."admin.php?page=".$_REQUEST['page']?>"><?php _e('Reset','binary-mlm-pro');?></a>
</div>	
	
<div style="margin-right:10px;float:right">			
	<form action="" method="get" style="float:right;">
		<input type="hidden" name="page" value="admin-mlm-epin-reports" />
		<input type="hidden" name="tab" value="epinreport" />
...
<?php  
$status=!empty($epin_status)? " status='$epin_status'":"";
$usersearch =!empty($search)? " epin_no like '%".trim($search)."%'" : '';
$where=!empty($status)||!empty($usersearch) ? ' WHERE' : '';
$and=!empty($status)&&!empty($usersearch) ? ' AND' : '';
//$status=" WHERE status=$epin_status";
$sql = "SELECT * FROM wp_mlm_epins  $where $status $and $usersearch ORDER BY id ASC";
//echo $sql;
$rs = mysql_query($sql);
$i = 0; $listArr = array(); 
